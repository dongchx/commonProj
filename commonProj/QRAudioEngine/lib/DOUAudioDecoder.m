...
// magicCookieData
/*
 magic cookie是附加在音频文件或者音频流中的一组不透明的元数据，
 而元数据给解码器提供了正确解码音频文件或音频流所必须的细节，
 获取的magicCookie要提供给AudioConvert使用
 */
- (void)_fillMagicCookieForAudioFileID:(AudioFileID)inputFile
{
  ...
  if (status == noErr && cookieSize > 0) {
    ...
    if (status != noErr) {
      ...
      // set MagicCookie for audioConverter
...
- (DOUAudioDecoderStatus)decodeOnce
{
  ...
  if (![provider isFinished]) {
    ...
    NSInteger receivedDataLength  = (NSInteger)([provider receivedLength] - dataOffset);// 正经的pcm音频数据长度
